...
def configure_hooks():
    ...
    # ── 自动启动 overlay（Claude Code 启动时拉起）──
    # 注意：只检查 .overlay.running，不检查 .statusd.running。
    # daemon 是持久进程，它的 marker 永远新鲜——如果同时检查，overlay 永远不会被拉起。
    ...
    # 文件时间检测：120 秒内有更新 → overlay 进程存活，跳过启动
    _auto_start_cmd = (
        f'need_start=1; '
        f'm="{_marker_ov}"; '
        f'if [ -f "$m" ]; then '
